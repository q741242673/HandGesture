...
class CameraViewController: UIViewController {
    ...
    func setupAVSession() throws {
		...
		// どちら側のカメラかを指定してビデオデバイスを生成する
//		guard let videoDevice = AVCaptureDevice.default(.builtInWideAngleCamera, for: .video, position: .back) else {	// 裏面カメラ
		guard let videoDevice = AVCaptureDevice.default(.builtInWideAngleCamera, for: .video, position: .front) else {	// セルフィーカメラ
